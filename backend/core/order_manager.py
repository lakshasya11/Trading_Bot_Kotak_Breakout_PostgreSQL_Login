    ...
    async def execute_basket_order(self, quantity, transaction_type, tradingsymbol, exchange, freeze_limit=None, price=None, product=None, use_chase=False, chase_retries=4, chase_timeout_ms=250, use_level2_flow=True, use_dynamic_freeze_limit=True, fallback_to_market=True, prefetched_depth_task=None, no_wick_percentage_mode=False, no_wick_depth_mode=False, max_slippage_percent=2.5):
        ...
        if len(orders_list) == 1:
            ...
            if price is None or price <= 0:
                ...
                # ⚡ OPT3: REDUCED RETRIES — 1 aggressive IOC + immediate MARKET fallback
                # Old: 3 retries (IOC₀.₂₅ → 100ms sleep → IOC₀.₅₀ → 100ms sleep → MARKET) = ~1000ms worst case
                # New: 2 steps  (IOC₀.₅₀ → MARKET) = ~550ms worst case
                # Buffer lifted to ₹0.50 on first attempt to maintain high fill rate with same slippage
                if no_wick_depth_mode:
                    max_level2_retries = 1   # NO-WICK: always MARKET, only 1 iteration needed
                else:
                    max_level2_retries = 2   # OPT3: IOC at ₹0.50 + MARKET (was 3)
                status = "FAILED"
                order_id = None
                ...
                for retry_attempt in range(max_level2_retries):
                    # ⚡ OPT3: Simplified 2-step execution
                    if no_wick_depth_mode:
                        # NO-WICK FAST PATH: Always MARKET (guaranteed fill, no IOC waste)
                        ioc_timeout_ms = None
                        use_market_fallback = True
                    elif retry_attempt == 0:
                        # ⚡ OPT3 Attempt 1: Single aggressive IOC at ₹0.50 buffer (merged old attempts 1+2)
                        # ₹0.50 buffer gives great fill rate without excessive slippage
                        ioc_timeout_ms = 400
                        use_market_fallback = False
                    else:
                        # ⚡ OPT3 Attempt 2: Skip IOC, go straight to MARKET (guaranteed fill)
                        # No 100ms sleep before this — MARKET fires immediately after IOC miss
                        ioc_timeout_ms = None
                        use_market_fallback = True
