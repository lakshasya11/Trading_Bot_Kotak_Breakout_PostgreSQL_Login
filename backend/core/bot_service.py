            ...
            try:
                ...
                if expiry_str:
                    try:
                        ...
                        # 🔧 FIX: Check if the configured expiry has already expired
                        # If so, auto-fall-back to the nearest available future expiry
                        if parsed_date < date_cls.today():
                            print(f"⚠️ Configured expiry {expiry_str} has ALREADY EXPIRED (today={date_cls.today()}).")
                            print(f"   Auto-detecting nearest valid expiry from scripmaster...")
                            auto_expiry = self.strategy_instance.get_selected_expiry()
                            if auto_expiry:
                                self.strategy_instance.last_used_expiry = auto_expiry
                                print(f"✅ Auto-selected nearest expiry: {auto_expiry} (was {expiry_str})")
                            else:
                                print(f"❌ No future expiry found in scripmaster for {self.strategy_instance.index_symbol}. Option chain will be empty.")
