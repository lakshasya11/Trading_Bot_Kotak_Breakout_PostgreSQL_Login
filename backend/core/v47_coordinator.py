...
class V47StrategyCoordinator:
    ...
    async def continuous_monitoring(self):
        ...
        # ⚡ OPT1: BACKGROUND DEPTH PRE-FETCH (saves 100-200ms at signal time)
        # Pre-fetch ATM option depth every 500ms so it's ready when a signal fires
        current_time_opt = time.time()
        if not hasattr(self, '_last_depth_prefetch_time'):
            self._last_depth_prefetch_time = 0
            self._prefetched_depth_cache = {}
        
        if current_time_opt - self._last_depth_prefetch_time >= 0.5:
            self._last_depth_prefetch_time = current_time_opt
            try:
                ...
                if spot:
                    ...
                    trend = self.strategy.data_manager.trend_state
                    # Pre-fetch whichever side the trend suggests
                    if trend == 'BULLISH':
                        opt = self.strategy.get_entry_option('CE', atm_strike)
                    else:
                        opt = self.strategy.get_entry_option('PE', atm_strike)
                    
                    if opt:
                        sym = opt['tradingsymbol']
                        exch = self.strategy.exchange
                        full_sym = f"{exch}:{sym}"
                        from .broker_factory import broker as _kite
                        # Fire-and-forget background fetch; store result for use at entry
                        async def _do_prefetch(fs=full_sym, s=sym):
                            try:
                                import asyncio as _asyncio
                                q = await _asyncio.wait_for(_kite.quote([fs]), timeout=0.15)
                                if q and fs in q:
                                    self._prefetched_depth_cache[s] = {
                                        'data': q[fs],
                                        'time': time.time()
                                    }
                            except Exception:
