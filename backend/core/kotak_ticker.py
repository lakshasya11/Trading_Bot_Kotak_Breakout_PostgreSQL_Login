        ...
        while not self.manual_stop:
            try:
                ...
                if instruments:
                    ...
                    # 🔧 FIX: Persistent zero-price diagnostic (fires every 60s if options still at 0)
                    # Helps diagnose: market closed, bad subscription, Kotak API returning zeros
                    if len(instruments) > 2 and zero_count > len(instruments) * 0.8:
                        if not hasattr(self, '_zero_price_warn_count'):
                            self._zero_price_warn_count = 0
                        self._zero_price_warn_count += 1
                        # Log every 120 polls (~60 seconds at 500ms interval)
                        if self._zero_price_warn_count % 120 == 1:
                            print(f">>> KOTAK TICKER: ⚠️ {zero_count}/{len(instruments)} instruments "
                                  f"have LTP=0. Option chain will show '--'. "
                                  f"If market hours: check subscription/login. "
                                  f"If after hours: normal (options have no live price).")
                    else:
                        # Reset counter when prices start flowing
                        if hasattr(self, '_zero_price_warn_count') and self._zero_price_warn_count > 0:
                            self._zero_price_warn_count = 0
                            print(f">>> KOTAK TICKER: ✅ Option prices flowing normally ({len(ticks)} instruments with LTP>0)")
