...
class Strategy:
    ...
    async def take_trade(self, trigger, opt, custom_entry_price=None, momentum_data=None, signal_generation_time=None):
        ...
        async with self.position_lock:
            ...
            async def check_layer6():
                ...
                # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                # BYPASS 2: Entry Price Crossed = Trend Proven, INSTANT Re-Entry
                # If price returns above original entry price, the original signal
                # was right and the SL exit was noise — re-enter immediately.
                # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                last_original_entry_price = last_exit_data.get('entry_price')
                if current_price and last_original_entry_price and current_price >= last_original_entry_price:
                    await self._log_debug("Layer 6 BYPASS",
                        f"🚀 ENTRY PRICE RECOVERY: ₹{last_exit_price:.2f} exit → price ₹{current_price:.2f} crossed above original entry ₹{last_original_entry_price:.2f}! "
                        f"Trend proven, INSTANT re-entry!!")
                    del self.symbol_exit_cooldown[symbol]
                    return True, None

                # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                # BYPASS 2.5: Price Recovered 0.5%+ above exit price = Early Re-Entry
                # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                if current_price and last_exit_price:
                    recovery_pct = ((current_price - last_exit_price) / last_exit_price) * 100
                    if recovery_pct >= 0.5:  # Lowered from 1% → 0.5% for faster re-entry
                        await self._log_debug("Layer 6 BYPASS",
            ...
            try:
                ...
                if self.params.get("trading_mode") == "Live Trading":
                    ...
                    async def place_order_task():
                        ...
                        if no_wick_bypass:
                            ...
                            # ⚡ OPT2: HIGH-VELOCITY FAST PATH — direct MARKET, no depth/IOC overhead
                            # If option velocity >= ₹1/s the move is strong & fast; MARKET fill is optimal
                            try:
                                opt_velocity = self.calculate_price_velocity(symbol, lookback_seconds=1.5)
                            except Exception:
                                opt_velocity = 0.0
                            
                            if opt_velocity >= 1.0:
                                await self._log_debug("OPT2-FastEntry",
                                    f"⚡ HIGH VELOCITY ₹{opt_velocity:.2f}/s on {symbol} → MARKET order (skip IOC retries)")
                                return await self.order_manager.execute_basket_order(
                                    quantity=qty,
                                    transaction_type=kite.TRANSACTION_TYPE_BUY,
                                    tradingsymbol=symbol,
                                    ...
                                    product=kite.PRODUCT_MIS,
                                    price=price,
                                    use_level2_flow=False,   # Skip depth fetch entirely
                                    use_chase=False,
                                    chase_retries=0,
                                    chase_timeout_ms=0,
                                    no_wick_depth_mode=True,  # Goes to MARKET immediately
                                    max_slippage_percent=2.5,
                                )
                            # END OPT2
                            
                            # ✅ STANDARD ENTRY: 3 attempts with depth analysis
                            return await self.order_manager.execute_basket_order(
                                quantity=qty, 
                                transaction_type=kite.TRANSACTION_TYPE_BUY, 
                                tradingsymbol=symbol, 
                ...
                if self.ticker_manager:
                    ...
                    elif instrument_token < 100000:
                        await self._log_debug("ERROR", 
                            f"❌ CRITICAL: Suspiciously low instrument token {instrument_token} for {symbol}. This is likely wrong (valid tokens > 1M)!")
                    else:
    ...
    async def exit_position(self, reason, skip_layer6_cooldown=False):
        ...
        try:
            ...
            self.symbol_exit_cooldown[p["symbol"]] = {
                'time': now,
                'price': exit_price,
                'entry_price': p.get('entry_price', exit_price),  # ✅ Store original entry price for recovery bypass
                'candle_start_time': candle_start_time,
                'direction': p["direction"],
                'reason': reason
            }
            
            # 🔧 OPTION D REMOVED: symbol_entry_cooldown was a redundant 15s block on top of
            # symbol_exit_cooldown. The exit cooldown already prevents duplicates with 3 smart
            # bypasses (price recovery 0.5%, entry price crossed, new candle). Removing this
            # allows re-entry within the same minute when price genuinely moves.
            await self._log_debug("Layer 6",
                f"✅ Exit recorded for {p['symbol']} (Exit: {reason}) — re-entry governed by symbol_exit_cooldown")
        ...
        try:
            ...
            for tick in ticks:
                ...
                if token is not None and ltp is not None and (symbol := self.token_to_symbol.get(token)):
                    ...
                    # 🔧 FIX: Mark chain dirty on EVERY option tick (not just index ticks)
                    # Previously the chain only refreshed on index ticks, but option prices
                    # may arrive slightly later → chain showed '--' until next index tick
    ...
    async def _update_ui_option_chain(self):
        ...
        # 🔧 AUTO-HEAL: If ALL strikes show '--', try a background bootstrap to fetch prices
        # This handles: startup before first option tick, Kotak 0-price responses, subscription gaps
        import time as _chain_time
        all_missing = data and all(row.get("ce_ltp") == "--" and row.get("pe_ltp") == "--" for row in data)
        if all_missing:
            last_retry = getattr(self, '_last_bootstrap_retry', 0)
            if _chain_time.time() - last_retry > 10:   # Retry at most once every 10 seconds
                self._last_bootstrap_retry = _chain_time.time()
                asyncio.create_task(self._bootstrap_option_prices())
    ...
    async def load_instruments(self):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # 🔧 FIX: Filter option_instruments to ONLY include the selected index.
                # Previously ALL 108434 instruments were stored, causing FINNIFTY/stock options
                # to appear when NIFTY was selected (same strike + expiry matched wrong instrument).
                if matching_instruments:
                    self.option_instruments = matching_instruments
                    await self._log_debug("Instruments", f"✅ option_instruments filtered to {len(matching_instruments)} {self.index_name} instruments")
                elif instruments:
                    # Fallback: store all if no matching instruments found (avoids empty chain)
                    self.option_instruments = instruments
                    await self._log_debug("Instruments", f"⚠️ No instruments matched '{self.index_name}' — storing all {len(instruments)} as fallback")
                else:
                    self.option_instruments = []
                    await self._log_debug("Instruments", f"⚠️ No instruments available — option chain will be empty")
