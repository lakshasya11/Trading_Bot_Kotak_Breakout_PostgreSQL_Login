...
class KotakBroker(BrokerInterface):
    ...
    # ── Transient Kotak error codes that warrant a TOTP retry ──────────
    _TRANSIENT_LOGIN_ERRORS = {"10010", "service error", "internal server"}

    @staticmethod
    def _is_transient_login_error(error_msg: str) -> bool:
        """Return True if the error is a transient server-side failure worth retrying."""
        msg_lower = str(error_msg).lower()
        return any(code in msg_lower for code in KotakBroker._TRANSIENT_LOGIN_ERRORS)

    def generate_session(self, request_token=None, api_secret=None):
        """
        Two-step Kotak Neo v2 login:
          1. TOTP Login  → viewToken + viewSid  (retried up to 3x on transient errors)
          2. MPIN Validate → session token (Auth) + session sid (Sid) + baseUrl

        Retries are only triggered for transient Kotak server errors (e.g. 10010).
        Credential / MPIN errors fail immediately without retrying.
        """
        ...
        # ── Step 1: TOTP Login (with retry for transient errors) ────────
        _MAX_TOTP_ATTEMPTS = 3
        _TOTP_RETRY_WAIT = 30  # seconds — matches TOTP code rotation window

        view_token = None
        view_sid = None

        for attempt in range(1, _MAX_TOTP_ATTEMPTS + 1):
            # Generate a fresh TOTP code on every attempt
            if totp_secret:
                totp_code = _generate_totp(totp_secret)
                print(f"[KOTAK] Step 1: TOTP login for {ucc} "
                      f"(attempt {attempt}/{_MAX_TOTP_ATTEMPTS}, auto-generated)...")
            else:
                if attempt == 1:
                    print(f"\n{'='*60}")
                ...
                print(f"[KOTAK] Step 1: TOTP login for {ucc} "
                      f"(attempt {attempt}/{_MAX_TOTP_ATTEMPTS})...")
            ...
            view_token = (step1.get("data", {}).get("token")
                          if isinstance(step1.get("data"), dict)
                          else step1.get("token"))
            view_sid = (step1.get("data", {}).get("sid")
                        if isinstance(step1.get("data"), dict)
                        else step1.get("sid"))
            ...
            # Login failed — decide whether to retry
            error_msg = step1.get("message") or step1.get("error") or str(step1)

            if not self._is_transient_login_error(str(error_msg)):
                # Permanent error (wrong credentials, invalid MPIN, etc.) — don't retry
                raise Exception(f"TOTP login failed: {error_msg}")

            if attempt < _MAX_TOTP_ATTEMPTS:
                print(f"[KOTAK] Step 1 failed (attempt {attempt}/{_MAX_TOTP_ATTEMPTS}): "
                      f"{error_msg}. "
                      f"Transient server error — waiting {_TOTP_RETRY_WAIT}s for TOTP refresh...")
                time.sleep(_TOTP_RETRY_WAIT)
            else:
                raise Exception(f"TOTP login failed after {_MAX_TOTP_ATTEMPTS} attempts: {error_msg}")

        if not view_token or not view_sid:
            raise Exception("TOTP login failed: No token/sid returned.")
