...
class DataManager:
    ...
    async def on_new_minute(self, new_minute_ltp):
        ...
        # ✅ FIX: Use IST time (not UTC) so index candles align to market minutes
        current_ist = get_ist_time()
        current_ist_minute = current_ist.replace(second=0, microsecond=0)
        aligned_start_utc = current_ist_minute.astimezone(timezone.utc)
        candle_start_unix = calendar.timegm(aligned_start_utc.timetuple())
        self.current_candle = {"minute": current_ist_minute, "open": new_minute_ltp, "high": new_minute_ltp, "low": new_minute_ltp, "close": new_minute_ltp, "candle_start_time": candle_start_unix}
    ...
    def update_live_candle(self, ltp, symbol=None):
        ...
        # ✅ FIX: Use IST time (not UTC) for minute boundary — candles must align to market minutes
        current_ist = get_ist_time()
        current_dt_minute = current_ist.replace(second=0, microsecond=0)
        ...
        if not is_new_minute and "open" in candle_dict: 
            ...
            if not is_index:
                import calendar
                # ✅ FIX: candle_start_time = floor of current IST minute as Unix timestamp
                # This is consistent regardless of when the poll tick arrives (up to 500ms jitter)
                aligned_minute_start = current_dt_minute.astimezone(timezone.utc)
                candle_start_unix = calendar.timegm(aligned_minute_start.timetuple())
                self.option_candles[symbol] = {"minute": current_dt_minute, "open": ltp, "high": ltp, "low": ltp, "close": ltp, "candle_start_time": candle_start_unix}
