      ...
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="padding: 30px; background: #f9f9f9; border-radius: 10px; margin-top: 20px;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="http://localhost:3001" style="background: linear-gradient(135deg, #b19cd9, #9575cd); color: white; padding: 12px 30px; text-decoration: none; border-radius: 25px; font-weight: bold;">Login Now</a>
          </div>
...
def send_login_email(to_email, first_name, client_id, index=''):
    sender_email = os.getenv('SMTP_EMAIL')
    password = os.getenv('SMTP_PASSWORD')
    smtp_server = os.getenv('SMTP_SERVER')
    smtp_port = int(os.getenv('SMTP_PORT'))
    ...
    message = MIMEMultipart("alternative")
    message["Subject"] = "Trading Bot - Bot Started Alert"
    message["From"] = sender_email
    message["To"] = to_email
    ...
    <html>
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #10b981, #059669); padding: 30px; border-radius: 10px; text-align: center; color: white;">
          <h1 style="margin: 0;">🤖 Bot Started</h1>
        </div>
        <div style="padding: 30px; background: #f9f9f9; border-radius: 10px; margin-top: 20px;">
          <h2 style="color: #333;">Hello {first_name},</h2>
          <p style="color: #666;">Your Trading Bot has been started successfully.</p>
          <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #10b981; margin: 20px 0;">
            <p><strong>Client ID:</strong> {client_id}</p>
            <p><strong>Index:</strong> {index if index else 'N/A'}</p>
            <p><strong>Start Time:</strong> {start_time} IST</p>
          </div>
          <p style="color: #888; font-size: 0.9em;">If this wasn't you, please stop the bot and contact support immediately.</p>
        </div>
      </body>
    </html>
    """
    ...
    try:
        ...
        server.sendmail(sender_email, to_email, message.as_string())
        server.quit()
        print(f"Bot start email sent to {to_email}")
    except Exception as e:
        print(f"Login email error: {e}")


def send_logout_email(to_email, first_name, client_id, reason='Bot Stopped'):
    sender_email = os.getenv('SMTP_EMAIL')
    password = os.getenv('SMTP_PASSWORD')
    smtp_server = os.getenv('SMTP_SERVER')
    smtp_port = int(os.getenv('SMTP_PORT'))
    ...
    message = MIMEMultipart("alternative")
    message["Subject"] = f"Trading Bot - {reason} Alert"
    message["From"] = sender_email
    message["To"] = to_email
    ...
    <html>
      <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="padding: 30px; background: #f9f9f9; border-radius: 10px; margin-top: 20px;">
          <h2 style="color: #333;">Hello {first_name},</h2>
          <p style="color: #666;">Your Trading Bot has been stopped.</p>
          <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #ef4444; margin: 20px 0;">
            <p><strong>Client ID:</strong> {client_id}</p>
            <p><strong>Reason:</strong> {reason}</p>
            <p><strong>Stop Time:</strong> {stop_time} IST</p>
          </div>
          <p style="color: #888; font-size: 0.9em;">If this wasn't you, please contact support immediately.</p>
        </div>
      </body>
    </html>
    """
    ...
    try:
        ...
        server.sendmail(sender_email, to_email, message.as_string())
        server.quit()
        print(f"Bot stop email sent to {to_email} (reason: {reason})")
    except Exception as e:
        print(f"Logout email error: {e}")


@app.route('/api/send-bot-alert', methods=['POST'])
def send_bot_alert():
    ...
    reason = data.get('reason', 'Bot Stopped')

    if not email:
        return jsonify({'error': 'Email required'}), 400

    if event == 'started':
        send_login_email(email, name, client_id, index)
    elif event == 'stopped':
        send_logout_email(email, name, client_id, reason)
...
def logout():
    """Logout: clear token and disconnect WebSockets via trading backend."""
    import requests as req
    trading_backend = 'http://localhost:8000'
    errors = []

    # Call trading backend logout (stops bot + clears token + disconnects WebSockets)
    try:
        req.post(f'{trading_backend}/api/logout', timeout=10)
    except Exception as e:
        errors.append(f'trading_logout: {e}')
    ...
    try:
        ...
        if users:
            ...
            send_logout_email(
                user.get('email'),
                user.get('firstName', user.get('client_id', 'User')),
                user.get('client_id', ''),
                'User Logged Out'
            )
    except Exception as e:
